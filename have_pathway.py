...
class FeatureEncoder(nn.Module):
    ...
    def set_SpecPath(self, path):
        self.spectral_encoder = path
...
class StatisticianModule(nn.Module):
    def __init__(self, dense, classes):
        ...
        self.c_dense = nn.Linear(int(64*2), dense)  # 64 * 2 + 64 * 2 + 60 * 2 = 376 = sum(i=3) represntation*2

        self.gap_pwconv = nn.Conv1d(int(64), dense, 1)
        self.gvp_pwconv = nn.Conv1d(int(64), dense, 1)

        self.fullconnect = nn.Sequential(
            nn.Dropout(0.5),
            nn.Linear(int(64), self.classes)
        )
...
specPath = StoppedBandPathway_1Dconv.StoppedBandPathway(fs=100,electrode=23,Unsupervise=False,encode_info = encode_info,bands=BANDS).to(device)

CrossEL = torch.nn.CrossEntropyLoss()

featureencoder = FeatureEncoder()
featureencoder.set_SpecPath(specPath)
statistic = StatisticianModule(376,2)
model = TIPNet(featureencoder, statistic).to(device)
