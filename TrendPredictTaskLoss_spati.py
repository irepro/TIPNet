...
class TemporalTrendIdentification_TaskLoss(torch.nn.modules.loss._Loss):
    ...
    def forward(self, CrossEL, batch, encoder, train, window=50, A=50, B=0.75, C=50, D=0.75):
        batch_aug = []
        batch_label = []
        ...
        for idx,band in enumerate(self.BAND):
            ...
            batch_label.append(temp.shape[0] * [self.LABEL[idx]])
        ...
        if train == TRAIN:
            ...
            acc = accuracy_check(pred.detach(), torch.Tensor(batch_label))
            return loss.item(), acc #, sample, np.array(dotprod)