...
class DEAPDataset(Dataset):
    def __init__(self, idxs, ssl = True, sfreq=200, da_idx=None, ratio = None):
        ...
        for idx in idxs:
            ...
            if ratio != None:
                ...
                if da_idx==None:
                    ...
                    labels.append([[da_idx]*32]*length)
            else:
                data.append(tmp['data'][:, :32, -30*128:])
                if da_idx==None:
                    labels.append(tmp['labels'])
                else :
                    labels.append([[da_idx]*32] * tmp['labels'].shape[0])

        del tmp

        data = np.moveaxis(np.concatenate(data, axis=0), -1, 0)
        data = signal.resample(data, int(30 * sfreq))
...
class SEEDDataset(Dataset):
    def __init__(self, idxs, ssl = True, sfreq=200, da_idx=None, rm_front = False, ratio = None):
...
class SEEDIVDataset(Dataset):
    def __init__(self, idxs, ssl = True, sfreq=200, da_idx=None, rm_front = True, ratio = None):
        ...
        for idx in idxs:
            ...
            if ratio != None:
                ...
                if da_idx == None:
                    ...
                    labels.append(np.array([[da_idx]*23]*length))
            else:
                data.append(tmp)
                ...
                if da_idx == None:
                    ...
                    labels.append(np.array([[da_idx]*23]*len(y)))
...
class MASSDataset(Dataset):
    def __init__(self, idx, ssl=True, sfreq=200, da_idx=None, ratio=None):
        self.idx = idx
...
class ConcatDataInit():
    def __init__(self, idxs, ssl = True, sfreq=200):
        ...
        for idx, values in idxs.items():
            ...
            for name, value in values.items():
                ...
                if len(value) == 1:
                    if name == "SEED":
                        temp_dataset = SEEDDataset(value, ssl, sfreq, da_idx)
                    elif name == "SEEDIV":
                        temp_dataset = SEEDIVDataset(value, ssl, sfreq, da_idx)
                    elif name == "DEAP":
                        temp_dataset = DEAPDataset(value, ssl, sfreq, da_idx)
                    elif name == "CHB-MIT":
                        temp_dataset = CHB_MIT_Dataset(value, ssl, sfreq, da_idx)
                    elif name == "Sleepdef":
                        temp_dataset = SleepdefDataset(value, ssl, sfreq, da_idx)
                    elif name == "MASS":
                        temp_dataset = MASSDataset(value, ssl, sfreq, da_idx)
                else:
                    dem, ratio = value[0], value[1]
                    if name == "SEED":
                        temp_dataset = SEEDDataset(dem, ssl, sfreq, da_idx, ratio = ratio)
                    elif name == "SEEDIV":
                        temp_dataset = SEEDIVDataset(dem, ssl, sfreq, da_idx, ratio = ratio)
                    elif name == "DEAP":
                        temp_dataset = DEAPDataset(dem, ssl, sfreq, da_idx, ratio = ratio)
                    elif name == "CHB-MIT":
                        temp_dataset = CHB_MIT_Dataset(dem, ssl, sfreq, da_idx, ratio = ratio)
                    elif name == "Sleepdef":
                        temp_dataset = SleepdefDataset(value, ssl, da_idx, sfreq)
                    elif name == "MASS":
                        temp_dataset = MASSDataset(value, ssl, da_idx, sfreq)
