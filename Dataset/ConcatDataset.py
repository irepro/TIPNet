...
class SEEDDataset(Dataset):
    def __init__(self, idxs, ssl = True, sfreq=200, rm_front = False, ratio = None):
...
class SEEDIVDataset(Dataset):
    def __init__(self, idxs, ssl = True, sfreq=200, rm_front = True, ratio = None):
    ...
    def __init__(self, idxs, ssl = True, sfreq=200):
        ...
        for idx, values in idxs.items():
            for name, value in values.items():
                ...
                if len(value) == 1:
                    if name == "SEED":
                        temp_dataset = SEEDDataset(value, ssl, sfreq)
                    elif name == "SEEDIV":
                        temp_dataset = SEEDIVDataset(value, ssl, sfreq)
                    elif name == "DEAP":
                        temp_dataset = DEAPDataset(value, ssl, sfreq)
                    elif name == "CHB-MIT":
                        temp_dataset = CHB_MIT_Dataset(value, ssl, sfreq)
                    elif name == "Sleepdef":
                        temp_dataset = SleepdefDataset(value, ssl, sfreq)
                    elif name == "MASS":
                        temp_dataset = MASSDataset(value, ssl, sfreq)
                else:
                    dem, ratio = value[0], value[1]
                    if name == "SEED":
                        temp_dataset = SEEDDataset(dem, ssl, sfreq, ratio = ratio)
                    elif name == "SEEDIV":
                        temp_dataset = SEEDIVDataset(dem, ssl, sfreq, ratio = ratio)
                    elif name == "DEAP":
                        temp_dataset = DEAPDataset(dem, ssl, sfreq, ratio = ratio)
                    elif name == "CHB-MIT":
                        temp_dataset = CHB_MIT_Dataset(dem, ssl, sfreq, ratio = ratio)
                    elif name == "Sleepdef":
                        temp_dataset = SleepdefDataset(value, ssl, sfreq)
                    elif name == "MASS":
                        temp_dataset = MASSDataset(value, ssl, sfreq)
